...
## Network settings
net_schema = 'data'  # network schema
net_table = 'wbnet2012'  # network table
net_sql = 'bik_rest != 1 and inpdx = 1'  # (optional) body of SQL where clause to select network links
...
gmap_path = ('C:/Users/Bryan/OneDrive/_BikeAppProject/_MapMatching/ORcycle_archive/ORcycle/maps/')  # path to write gmap files
...
network = pg.Network(db_conn, net_schema + '.' + net_table,
                            length_attr='length_m', id_attr='psuid',
                            sql=net_sql, geom_attr='geom')
print 'Loading data...'
d = asap.Deployment(None, None, num=1,
                    gps=(gps_path, orcycle_gps),
                    astable_gps='trip_id')
index = 1
trip_id = None
while True:
    while trip_id not in d.gps:
        trip_id = raw_input('trip_id: ')
    #print pid
    gps = d.gps[trip_id]
    ...
    gps.screen(max_spd=40, max_accel=10, min_altitude=-100,
               max_altitude=9999, min_spd=0.45, debug=True)
    ...
    print '{} trips detected'.format(len(gps.trips))
    gps4326 = pg.GPSCollection(srid=2838,
                               points=[copy.copy(p) for p in gps.points])
    ...
    for trip in gps.trips:
        ...
        if route:
            ...
            print ('{:.1f} ms/pt (expected performance is 20-50 ms/pt)'
                   .format((time.time() - t1) / len(trip.gps.points) * 1000))
...
##            d.write_psql(db_conn, 'data',
##                         GPStripstages='orc_gpstrips',
##                         GPSroutes='orc_gpsroutes', GPSlinks='orc_gpslinks')
        tripnum += 1
    trip_id = None
    if psql_output:
        d.write_psql(db_conn, 'data',
